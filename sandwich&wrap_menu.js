...
function renderProductCard(item) {
    ...
    '<div class="card-content">' +
        '<div class="card-header">' +
            ...
            '</h2>' +
            '<span class="card-badge"><i class="fas fa-star"></i><b> Premium</b></span>' +
        '</div>' +
...
async function loadProducts() {
    ...
    try {
        const res = await fetch(API_BASE + '/products/list.php?category=wraps');
        const data = await res.json();
        const cacheKey = JSON.stringify(data.items || []);

        if (cacheKey === productsDataCache) return;
        ...
        if (data.items && data.items.length) {
            ...
            data.items.forEach(function(item) {
                ...
                card.innerHTML = renderProductCard(item);
